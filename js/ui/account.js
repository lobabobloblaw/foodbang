/* DoorGorge — account, settings, addresses, payments, notifications */
window.DG = window.DG || {};
(function (DG) {
  'use strict';

  /* ===================== account root ===================== */
  DG.screens.register('account', {
    tab: 'account',
    appbar: function () {
      return '<div class="bar"><h1>Account</h1>' +
        '<button class="iconbtn" data-go="settings" aria-label="Settings">' + DG.icon('settings', 19) + '</button></div>';
    },
    render: function () {
      var st = DG.S();
      ...
      h += '<div class="acct-head"><img src="' + DG.esc(st.user.avatar) + '" alt="" onerror="this.style.visibility=\'hidden\'">' +
        '<div class="ah-b"><h1>' + DG.esc(st.user.name) + '</h1>' +
        '<span>' + DG.esc(st.user.email) + '</span>' +
      ...
      h += '<div class="statgrid">' +
        ...
        '<div><b>' + DG.money(st.meta.lifetimeSpend) + '</b><span>SPENT</span></div>' +
        '<div><b style="color:var(--gorge)">' + DG.money(st.meta.lifetimeFees) + '</b><span>IN FEES</span></div></div>';

      /* GORGE+ card */
      if (st.plus.active) {
        h += '<button class="plusbanner" data-go="plus" style="margin-top:16px">' +
          '<span class="pb-k">GORGE+ INFINITY PRIME ELITE · ACTIVE</span>' +
          '<span class="pb-t">Renews ' + DG.esc(st.plus.renewsOn || 'monthly') + '</span>' +
          '<span class="pb-s">You have saved ' + DG.money(st.plus.saved || 0) + ' since joining. Membership has cost you ' +
          DG.money(19.99 * Math.max(1, Math.round((Date.now() - (st.plus.since || Date.now())) / 2592000000) || 1)) + '.</span>' +
          '<span class="pb-c">Manage membership</span></button>';
      } else {
        h += '<button class="plusbanner" data-go="plus" style="margin-top:16px">' +
          '<img src="assets/app/gorgeplus-hero.webp" alt="" onerror="this.remove()">' +
          '<span class="pb-k">GORGE+ INFINITY PRIME ELITE</span>' +
          '<span class="pb-t">You have paid ' + DG.money(st.meta.lifetimeFees) + ' in fees</span>' +
          '<span class="pb-s">GORGE+ would have reduced some of them. $19.99/month.</span>' +
      ...
      h += '<button class="mrow" data-go="bodymax" style="border-top:8px solid var(--surface-2);border-bottom:1px solid var(--line);margin-top:14px">' +
        DG.icon('activity', 19) + '<span class="mr-b"><b>BODYMAX™ Intake Telemetry</b>' +
        '<span>' + (m.orders ? DG.int(m.totalCal) + ' units logged · trajectory ' + m.trajectory : 'No signal yet') + '</span></span>' +
        '<span class="mr-r">' + (m.orders ? '<span class="badge badge--bad">' + m.trajectory + '</span>' : '') + DG.icon('fwd', 15) + '</span></button>';

      h += '<div class="menulist"><h3>ORDERING</h3>' +
        row('pin', 'Addresses', DG.plural(st.addresses.length, 'saved address', 'saved addresses'), 'addresses') +
        row('card', 'Payment methods', DG.plural(st.payments.length, 'method'), 'payments') +
        row('orders', 'Order history', DG.plural(st.orders.length, 'order'), 'orders') +
        row('heart', 'Saved stores', DG.plural(st.favorites.length, 'store'), 'favorites') +
        row('tag', 'Promotions', Object.keys(DG.fees.PROMOS).length + ' codes circulating', null, 'promos') +
        '</div>';
      ...
      h += '<div class="menulist"><h3>DANGER</h3>' +
        '<button class="mrow" data-reset>' + DG.icon('trash', 19) +
        ...
        '<button class="mrow" data-signout>' + DG.icon('logout', 19) +
        '<span class="mr-b"><b>Sign out</b><span>You cannot sign out. This is a demonstration.</span></span></button>' +
        '</div>';

      h += '<div class="fineprint">DoorGorge™ v9.4.1 (build 40118) · Nutrition Logistics, Inc.<br>' +
    ...
    mount: function (root) {
      DG.on(root, 'click', '[data-act="notifs"]', DG.openNotifications);
      DG.on(root, 'click', '[data-act="promos"]', openPromos);
      DG.on(root, 'click', '[data-act="help"]', openHelp);
      DG.on(root, 'click', '[data-act="legal"]', openLegal);
      DG.on(root, 'click', '[data-editprofile]', openProfile);
      DG.on(root, 'click', '[data-signout]', function () {
        DG.toast('Sign-out is not available in this build. It is available in a future build.');
      });
      DG.on(root, 'click', '[data-reset]', function () {
        DG.confirm({ title: 'Reset all data?', danger: true, yes: 'Erase everything', no: 'Keep it',
          body: 'This clears every order, cart, achievement and setting stored in this browser. It cannot be undone, and it is free.' })
          .then(function (ok) { if (ok) { DG.store.reset(); DG.applyAppearance(); DG.nav.tab('home'); DG.toast('All local data erased.'); } });
      });
    },
  });
  ...
  function row(icon, title, sub, go, act) {
    return '<button class="mrow"' + (go ? ' data-go="' + go + '"' : '') + (act ? ' data-act="' + act + '"' : '') + '>' +
      DG.icon(icon, 19) + '<span class="mr-b"><b>' + DG.esc(title) + '</b><span>' + DG.esc(sub) + '</span></span>' +
      '<span class="mr-r">' + DG.icon('fwd', 15) + '</span></button>';
  }

  /* ===================== favorites ===================== */
  DG.screens.register('favorites', {
    tab: 'account',
    appbar: function () { return '<div class="bar bar--border"><button class="iconbtn" data-back>' + DG.icon('back', 20) + '</button><h1>Saved stores</h1></div>'; },
    render: function () {
      var favs = DG.S().favorites.map(DG.catalog.get).filter(Boolean);
      if (!favs.length) return DG.C.empty({ title: 'Nothing saved', body: 'Saving a store lets us remind you about it more often.', cta: 'Browse', go: 'home' });
      return '<div class="grid1" style="padding-top:14px">' + favs.map(DG.C.storeCard).join('') + '</div><div style="height:24px"></div>';
    },
  });

  /* ===================== settings ===================== */
  DG.screens.register('settings', {
    tab: 'account',
    appbar: function () { return '<div class="bar bar--border"><button class="iconbtn" data-back>' + DG.icon('back', 20) + '</button><h1>Settings</h1></div>'; },
    render: function () {
      var s = DG.S().settings;
      var h = '';
      ...
      h += '<div class="menulist"><h3>ORDERING</h3>' +
        ...
        '<span style="font:700 15px var(--mono);color:var(--gorge)">' + s.hungerLevel + ' / 10</span></div>' +
        ...
        DG.esc(hungerCopy(s.hungerLevel)) + '</div></div>' +
      ...
      h += '<div class="menulist"><h3>PRIVACY &amp; DISCLOSURE</h3>' +
        ...
        '<div class="callout callout--warn" style="margin-top:6px">' + DG.icon('alert', 17) +
      ...
      h += '<div class="menulist"><h3>NOTIFICATIONS</h3>' +
        ...
        sw('n.gorgerMessages', n.gorgerMessages, 'Gorger messages', 'Messages from the person currently holding your food.') +
    ...
    mount: function (root) {
      DG.on(root, 'click', '[data-seg]', function (e, t) {
        var k = t.dataset.seg, v = t.dataset.v;
        DG.store.set(function (st) { st.settings[k] = v; return st; });
        if (k === 'theme' || k === 'textsize') DG.applyAppearance();
        if (k === 'language' && v === 'en-US-LOUD') DG.toast('LANGUAGE SET TO AMERICAN (LOUD).');
        DG.nav.refresh();
      });
      DG.on(root, 'click', '[data-sw]', function (e, t) {
        var k = t.dataset.sw;
        ...
        var s = DG.S().settings;
        if (k === 'motion') DG.applyAppearance();
        if (k === 'feeTransparency') DG.toast(s.feeTransparency ? 'Itemised fees restored. Fee Transparency Fee reinstated.' : 'Fees hidden. Fee Opacity Fee applied — it is larger.', { kind: 'bad' });
        if (k === 'reduceUpsells' && s.reduceUpsells) DG.toast('Recommendations reduced. Upsell Suppression Fee applied.', { kind: 'bad' });
        if (k === 'dataSharing' && !s.dataSharing) DG.toast('Data sharing off. Data Sovereignty Fee applied.', { kind: 'bad' });
        DG.nav.refresh();
      });
      DG.on(root, 'input', '[data-hunger]', function (e, t) {
        DG.store.set(function (st) { st.settings.hungerLevel = Number(t.value); return st; }, { silent: true });
      ...
      DG.on(root, 'input', '[data-tipdef]', function (e, t) {
        DG.store.set(function (st) { st.settings.autoTipPct = Number(t.value); return st; }, { silent: true });
        var lab = t.previousElementSibling.querySelector('span');
  ...
  function sw(key, on, title, sub) {
    return '<button class="switchrow" data-sw="' + key + '" role="switch" aria-checked="' + !!on + '">' +
      '<span class="sr-b"><b>' + DG.esc(title) + '</b><span>' + sub + '</span></span>' +
      '<span class="switch" aria-checked="' + !!on + '"></span></button>';
  }

  /* ===================== addresses ===================== */
  DG.screens.register('addresses', {
    tab: 'account',
    appbar: function () { return '<div class="bar bar--border"><button class="iconbtn" data-back>' + DG.icon('back', 20) + '</button>' +
      '<h1>Addresses</h1><button class="iconbtn" data-addnew>' + DG.icon('plus', 20) + '</button></div>'; },
    render: function () {
      var st = DG.S();
      return st.addresses.map(function (a) {
        return '<div class="mrow" style="align-items:flex-start">' + DG.icon('pin', 19) +
          '<span class="mr-b"><b>' + DG.esc(a.label) + (a.id === st.selectedAddress ? ' <span class="badge badge--good">In use</span>' : '') + '</b>' +
          '<span>' + DG.esc(a.line1) + (a.line2 ? ', ' + DG.esc(a.line2) : '') + '<br>' + DG.esc(a.city) + '</span>' +
      ...
      '<div style="padding:18px 16px"><button class="btn btn--ghost btn--block" data-addnew>' + DG.icon('plus', 17) + 'Add an address</button></div>' +
      '<div class="fineprint">Addresses are retained after deletion for operational purposes.</div>';
    },
    mount: function (root) {
      DG.on(root, 'click', '[data-use]', function (e, t) {
        DG.store.set(function (st) { st.selectedAddress = t.dataset.use; return st; });
        DG.toast('Delivery address updated.'); DG.nav.refresh();
      });
      DG.on(root, 'click', '[data-edit]', function (e, t) { editAddress(t.dataset.edit); });
      DG.on(root, 'click', '[data-del]', function (e, t) {
        if (DG.S().addresses.length < 2) { DG.toast('At least one address is required.'); return; }
        DG.confirm({ title: 'Delete address?', danger: true, yes: 'Delete', body: 'It will be removed from this list.' }).then(function (ok) {
          if (!ok) return;
          DG.store.set(function (st) {
      ...
      DG.on(root, 'click', '[data-addnew]', function () { editAddress(null); });
      DG.on(document.getElementById('appbar'), 'click', '[data-addnew]', function () { editAddress(null); });
    },
  });

  function editAddress(id) {
    var a = id ? DG.S().addresses.filter(function (x) { return x.id === id; })[0] : { label: '', line1: '', line2: '', city: '', instructions: '', dropoff: 'leave' };
    DG.sheet.open({
      ...
      html: field('Label', 'label', a.label, 'Home, Work, Other') +
        ...
        '<textarea class="textarea" data-f="instructions" placeholder="Gate code, floor, warnings…">' + DG.esc(a.instructions) + '</textarea></div>',
      ...
      onMount: function (b, h) {
        DG.on(h.el, 'click', '[data-save]', function () {
          var vals = {};
          DG.qsa('[data-f]', b).forEach(function (i) { vals[i.dataset.f] = i.value.trim(); });
          if (!vals.line1) { DG.toast('A street address is required.', { kind: 'bad' }); return; }
          DG.store.set(function (st) {
            if (id) {
          ...
          h.close(); DG.nav.refresh(); DG.toast('Address saved.');
        });
      },
    });
  }
  function field(label, key, val, ph) {
    return '<div class="field"><span class="lbl">' + label + '</span>' +
      '<input class="input" data-f="' + key + '" value="' + DG.attr(val || '') + '" placeholder="' + DG.attr(ph || '') + '"></div>';
  }

  DG.openAddressSheet = function (after) {
    var st = DG.S();
    DG.sheet.open({
      title: 'Deliver to',
      html: st.addresses.map(function (a) {
        return '<button class="opt" role="radio" aria-checked="' + (a.id === st.selectedAddress) + '" data-pick="' + a.id + '">' +
          '<span class="mark"></span><span class="opt-b"><b>' + DG.esc(a.label) + '</b>' +
          '<span>' + DG.esc(a.line1) + ' · ' + DG.esc(a.city) + '</span></span></button>';
      }).join(''),
      footer: '<button class="btn btn--ghost btn--block" data-manage>Manage addresses</button>',
      onMount: function (b, h) {
        DG.on(b, 'click', '[data-pick]', function (e, t) {
          DG.store.set(function (s) { s.selectedAddress = t.dataset.pick; return s; });
          h.close(); if (after) after(); else DG.nav.refresh();
        });
        DG.on(h.el, 'click', '[data-manage]', function () { h.close(); DG.nav.go('addresses'); });
      },
    });
  };

  /* ===================== payments ===================== */
  DG.screens.register('payments', {
    tab: 'account',
    appbar: function () { return '<div class="bar bar--border"><button class="iconbtn" data-back>' + DG.icon('back', 20) + '</button>' +
      '<h1>Payment methods</h1><button class="iconbtn" data-addcard>' + DG.icon('plus', 20) + '</button></div>'; },
    render: function () {
      var st = DG.S();
      return st.payments.map(function (p) {
        return '<div class="mrow" style="align-items:flex-start">' + DG.icon('card', 19) +
          '<span class="mr-b"><b>' + DG.esc(p.brand) + ' ····' + DG.esc(p.last4) +
          (p.id === st.selectedPayment ? ' <span class="badge badge--good">Default</span>' : '') + '</b>' +
          '<span>' + DG.esc(p.nickname) + ' · expires ' + DG.esc(p.exp) + '</span>' +
          '<span style="margin-top:6px;display:flex;gap:12px">' +
      ...
      '<div style="padding:18px 16px"><button class="btn btn--ghost btn--block" data-addcard>' + DG.icon('plus', 17) + 'Add payment method</button></div>' +
      '<div class="callout">' + DG.icon('lock', 17) + '<span>No payment information is transmitted anywhere. This screen is a simulation and accepts any digits.</span></div>';
    },
    mount: function (root) {
      DG.on(root, 'click', '[data-usep]', function (e, t) {
        DG.store.set(function (st) { st.selectedPayment = t.dataset.usep; return st; });
        DG.toast('Default payment updated.'); DG.nav.refresh();
      });
      DG.on(root, 'click', '[data-delp]', function (e, t) {
        if (DG.S().payments.length < 2) { DG.toast('At least one payment method is required.'); return; }
        DG.store.set(function (st) {
      ...
      DG.on(root, 'click', '[data-addcard]', add);
      DG.on(document.getElementById('appbar'), 'click', '[data-addcard]', add);
    },
  });
  ...
  function addCard() {
    DG.sheet.open({
      ...
      onMount: function (b, h) {
        DG.on(h.el, 'click', '[data-save]', function () {
          var v = {}; DG.qsa('[data-f]', b).forEach(function (i) { v[i.dataset.f] = i.value.trim(); });
          var digits = (v.num || '').replace(/\D/g, '');
          if (digits.length < 4) { DG.toast('Enter at least 4 digits.', { kind: 'bad' }); return; }
          DG.store.set(function (st) {
            st.payments.push({ id: DG.uid('p'), brand: digits[0] === '4' ? 'Visa' : digits[0] === '5' ? 'Mastercard' : 'GorgeCard',
              last4: digits.slice(-4), exp: v.exp || '01/30', nickname: v.nick || 'Card', isDefault: false });
            return st;
          });
          h.close(); DG.nav.refresh(); DG.toast('Card added.');
        });
      },
    });
  }

  DG.openPaymentSheet = function (after) {
    var st = DG.S();
    DG.sheet.open({
      ...
      html: st.payments.map(function (p) {
        return '<button class="opt" role="radio" aria-checked="' + (p.id === st.selectedPayment) + '" data-pickp="' + p.id + '">' +
          '<span class="mark"></span><span class="opt-b"><b>' + DG.esc(p.brand) + ' ····' + DG.esc(p.last4) + '</b>' +
          '<span>' + DG.esc(p.nickname) + '</span></span></button>';
      }).join(''),
      footer: '<button class="btn btn--ghost btn--block" data-managep>Manage payment methods</button>',
      onMount: function (b, h) {
        DG.on(b, 'click', '[data-pickp]', function (e, t) {
          DG.store.set(function (s) { s.selectedPayment = t.dataset.pickp; return s; });
          h.close(); if (after) after(); else DG.nav.refresh();
        });
        DG.on(h.el, 'click', '[data-managep]', function () { h.close(); DG.nav.go('payments'); });
      },
    });
  };
  ...
  function openProfile() {
    ...
    DG.sheet.open({
      ...
      onMount: function (b, h) {
        DG.on(h.el, 'click', '[data-save]', function () {
          var v = {}; DG.qsa('[data-f]', b).forEach(function (i) { v[i.dataset.f] = i.value.trim(); });
          DG.store.set(function (st) { Object.keys(v).forEach(function (k) { if (v[k]) st.user[k] = v[k]; }); return st; });
          h.close(); DG.nav.refresh(); DG.toast('Profile updated.');
        });
      },
    });
  }
  ...
  var NOTIFS = [
    ...
    ['bike', 'Rate your last Gorger', 'Ratings below 4 are shared with the Gorger and with you.', '1d'],
  ];
  DG.openNotifications = function () {
    DG.sheet.open({
      title: 'Notifications', sub: NOTIFS.length + ' unread · they are always unread',
      html: NOTIFS.map(function (n) {
        return '<div class="mrow" style="align-items:flex-start">' + DG.icon(n[0], 19) +
          '<span class="mr-b"><b>' + DG.esc(n[1]) + '</b><span>' + DG.esc(n[2]) + '</span></span>' +
      ...
      onMount: function (b, h) {
        DG.on(b, 'click', '[data-clearn]', function () { DG.toast('Marked as read. They will return.'); h.close(); });
      },
    });
  };
  ...
  function openPromos() {
    ...
    DG.sheet.open({
      ...
      html: Object.keys(DG.fees.PROMOS).map(function (k) {
        var p = DG.fees.PROMOS[k];
        return '<div class="mrow" style="align-items:flex-start">' + DG.icon('tag', 19) +
          '<span class="mr-b"><b>' + k + '</b><span>' +
          (p.kind === 'pct' ? Math.round(p.value * 100) + '% off the subtotal' : DG.money(p.value) + ' off the subtotal') +
          (p.min ? ' · requires ' + DG.money(p.min) + ' subtotal' : '') + '<br>' + DG.esc(p.blurb) + '</span></span></div>';
      }).join('') +
  ...
  function openHelp() {
    DG.sheet.open({
      ...
      html: [
        ...
        ['Where is my Gorger?', 'Your Gorger is where the map indicates, approximately, with a delay.'],
        ...
        return '<div class="mrow" style="align-items:flex-start">' + DG.icon('help', 19) +
          '<span class="mr-b"><b>' + DG.esc(r[0]) + '</b><span>' + DG.esc(r[1]) + '</span></span></div>';
      }).join(''),
  ...
  function openLegal() {
    DG.sheet.open({
      ...
      html: '<div style="padding:0 16px 30px;font:var(--t-sub);color:var(--ink-2);line-height:1.65">' +
        '<p><b style="color:var(--ink)">This application is a work of satire.</b> DoorGorge™, every restaurant in it, every menu item, ' +
        ...
        'DoorGorge™. Where these conflict, DoorGorge™ prevails.</p>' +
