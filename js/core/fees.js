...
(function (DG) {
  ...
  var WHY = {
    delivery: 'The Delivery Fee covers delivery. It does not cover the Gorger, who is covered by the Tip, which is optional in the same way that breathing is optional.',
    ...
    labor: 'Your Gorger will experience your order emotionally. This fee compensates DoorGorge™ for that experience.',
    ...
    express: 'Express Gorge places your order ahead of other orders, which are then placed ahead of yours.',
    plusBenefit: 'Realising a GORGE+ benefit requires a benefit realisation process.',
    other: 'Other.',
    ...
    tip: '100% of your tip goes to your Gorger, less the Tip Processing Fee, which is included in the Service Fee, which is not part of the tip.',
  };
  DG.FEE_WHY = WHY;

  function line(id, label, amount, note, kind) {
    return { id: id, label: label, amount: DG.round2(amount), note: note || null, kind: kind || 'fee' };
  }
  ...
  DG.fees = {
    ...
    compute: function (ctx) {
      var s = ctx.settings || DG.S().settings;
      var sub = DG.round2(ctx.subtotal || 0);
      ...
      /* ---------- delivery ---------- */
      if (mode === 'delivery') {
        var base = DG.round2(4.99 + Math.max(0, dist - 1.5) * 0.62);
        if (plus) {
          if (sub >= 312) {
            lines.push({ id: 'delivery', label: 'Delivery Fee', amount: 0, was: base, kind: 'fee', free: true,
              note: 'GORGE+ waives this on orders over $312.00.' });
          } else {
            lines.push(line('delivery', 'Delivery Fee (GORGE+ Reduced)', base * 0.7,
              'Full waiver applies at $312.00. You are ' + DG.money(312 - sub) + ' away.'));
          }
          lines.push(line('plusbenefit', 'GORGE+ Benefit Realization Fee', 1.99, null));
        } else {
          lines.push(line('delivery', 'Delivery Fee', base, null));
        }
        if (dist > 3.4) lines.push(line('range', 'Expanded Range Fee', 2.99, DG.round2(dist) + ' mi from the restaurant.'));
      } else {
      ...
      /* ---------- the stack ---------- */
      lines.push(line('service', 'Service Fee', Math.max(SERVICE_MIN, sub * SERVICE_PCT), '18.5% of subtotal, minimum ' + DG.money(SERVICE_MIN) + '.'));
      ...
      if (ctx.lineCount) lines.push(line('digitization', 'Menu Digitization Surcharge', 0.39 * ctx.lineCount, DG.plural(ctx.lineCount, 'line item') + ' × $0.39.'));
      if (mode === 'delivery') {
      ...
      if (ctx.express) lines.push(line('express', 'Express Gorge™', 5.99, 'Reduces estimated arrival by up to 1 minute.'));

      /* ---------- peak multiplier on the whole stack ---------- */
      var stack = DG.sum(lines, function (l) { return l.amount; });
      var peak = DG.round2(stack * (PEAK_MULT - 1));
      lines.push(line('peak', 'Peak Demand Multiplier ×' + PEAK_MULT.toFixed(1), peak, 'Applied to all fees above.'));

      var feesTotal = DG.round2(stack + peak);

      /* ---------- taxes ---------- */
      var taxable = DG.round2(sub - promoAmt + feesTotal);
      var tax = DG.round2(taxable * TAX_RATE);
      var otherFees = 1.15;
      var taxLine = line('taxes', 'Taxes & Other Fees', tax + otherFees, 'Includes ' + DG.money(otherFees) + ' in Other Fees.', 'tax');
      ...
      var tip = ctx.tipCustom != null ? DG.round2(ctx.tipCustom) : DG.round2(sub * (tipPct / 100));
      var tipLine = line('tip', 'Gorger Tip' + (tipPct != null ? ' (' + tipPct + '%)' : ' (custom)'), tip, null, 'tip');

      /* ---------- total + Convenience Rounding™ ---------- */
      var pre = DG.round2(sub - promoAmt + feesTotal + taxLine.amount + tip);
      var rounded = Math.ceil(pre / 5) * 5;
      var roundUp = DG.round2(rounded - pre);
      var roundLine = roundUp > 0.004 ? line('rounding', 'Convenience Rounding™', roundUp, 'Rounded up to the nearest $5.00.', 'fee') : null;
      var total = DG.round2(pre + (roundLine ? roundLine.amount : 0));

      return {
        subtotal: sub,
        discounts: discounts,
        promoAmount: DG.round2(promoAmt),
        feeLines: lines,
        stack: DG.round2(stack),
        peak: peak,
        feesTotal: feesTotal,
        taxLine: taxLine,
        tipLine: tipLine,
        ...
        nonFood: DG.round2(total - sub + promoAmt),
      };
    },
    ...
    tipTiers: function (sub) {
      return [
        { pct: 0,  label: 'No Tip',  sub: 'Your Gorger will be informed.' },
      ...
      ].map(function (t) { t.amount = DG.round2(sub * t.pct / 100); return t; });
    },
    ...
    checkPromo: function (code, sub) {
      ...
      if (!code) return null;
      var p = DG.fees.PROMOS[code];
      if (!p) return { code: code, valid: false, error: 'Code not recognised. It may have expired, or may never have existed.' };
      if (p.min && sub < p.min) return { code: code, valid: false, error: 'Requires a subtotal of ' + DG.money(p.min) + '. You are ' + DG.money(p.min - sub) + ' short.' };
